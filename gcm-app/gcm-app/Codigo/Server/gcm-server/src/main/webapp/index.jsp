<%-- 
    ...
    Description: Permite enviar mensagens para apenas uma pessoa cadastrada
                 no sistema.
--%>
                ...
                $("textarea").on({
                    ...
                    keyup: function() {
                        ...
                        if ($(textarea).val().length < 2048) {
                            ...
                            $(textarea).parent().find("input[type='submit']").fadeIn('fast');
                        }
                    },                    
                    paste: function(){
                        var textarea = $(this);
                        setTimeout(function(){
                            valideMensagem(textarea);
                        },100);
                    }
                });
            });
            
            function valideMensagem(campo){
                var textarea = $(campo);
                var spanContador = $($(textarea).parent().parent().find("span.badge")).find("span#contador");
                if ($(textarea).val().length < 1024) {
                    $(spanContador).text($(textarea).val().length);
                    $(campo).parent().find("input[type='submit']").fadeIn('fast');
                }
                    ...
                    $(campo).parent().find("input[type='submit']").fadeOut('fast');
                }
            }
