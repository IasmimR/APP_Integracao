...
/***
 * Classe responsável por cadastrar o usuário junto ao servidor GCM e enviar o
 * regId ao servidor JAVA.
 * 
...
public class RegisterActivity extends ActionBarActivity {
	...
	/**
	 * Referência ao edit text inputName.
	 */
	EditText mInputNome;

	/**
	 * Referência ao edit text inputEmail.
	 */
	EditText mInputEmail;

	/**
	 * Referência ao botão BtnRegistrar.
	 */
	Button mBtnRegistrar;

	/**
	 * Referência a API do GCM.
	 */
	...
	/**
	 * Variável para armazenar o regId obtido.
	 */
	String mRegId;

	/**
	 * Referência ao connection detector.
	 */
	ConnectionDetector mCnDetector;

	/**
	 * Nome da preference que contém o regId.
	 */
	public static final String REG_ID = "regId";

	/**
	 * Nome da preference que contém a versão do app.
	 */
	...
	/**
	 * Primeiro método chamado, para construir a interface e preparar gatilhos
			...
			public void onClick(View arg0) {
				if (TextUtils.isEmpty(mRegId)) {
					...
					Toast.makeText(getApplicationContext(),
							"Não há id GCM para este dispositivo.",
	...
	/**
	 * Método disparado para receber informações quando uma activity, disparada
	 * para obtenção de resultados retorna valor.
	 */
	@Override
	...
	/**
	 * Método para criar os menus da activity corrente.
	 */
	@Override
	...
	/**
	 * Gatilho para tratar quando uma opção e selecionada nos menus da activity.
	 */
	@Override
	public boolean onOptionsItemSelected(MenuItem item) {
		switch (item.getItemId()) {
		case R.id.action_exit:
			Toast.makeText(this, "Opção selecionada", Toast.LENGTH_SHORT)
					.show();
			break;
		}
	...
	/**
	 * Obtém a versão corrente do aplicativo para armazenar a mesma junto a
	 * sharedpreference.
	 * 
	 * @param context
	...
	private static int getAppVersion(Context context) {
		try {
			...
			Log.d("RegisterActivity", "Erro fatal na aplicação." + e);
		...
		new AsyncTask<Void, Void, String>() {
			...
			protected String doInBackground(Void... params) {
				...
				try {
					...
					Log.d(TAG, "registerInBackground - regId: " + mRegId);
					msg = mRegId;
				...
				Log.d(TAG, "AsyncTask completada: " + msg);
				return msg;
			}
			...
			protected void onPostExecute(String msg) {
				if (isSuccessed == true) {
					Toast.makeText(getApplicationContext(),
							R.string.toast_registration_id, Toast.LENGTH_LONG)
							.show();
				} else {
					Toast.makeText(getApplicationContext(),
							R.string.toast_registration_id_exception + msg,
							Toast.LENGTH_LONG).show();
				}
			}
	 ...
	 * 
	 * @param registredUser
	 *            Indica se o usuário já está registrado.
	 */
	private void startMainActivity(Boolean registredUser) {
		...
		if (registredUser) {
			...
			i.putExtra(EnumPreferences.NAME.toString(), Util.getPreference(
					getApplicationContext(), EnumPreferences.NAME.toString(),
					String.class));
			i.putExtra(EnumPreferences.EMAIL.toString(), Util.getPreference(
					getApplicationContext(), EnumPreferences.EMAIL.toString(),
					String.class));
		} else {
			i.putExtra("regId", mRegId);
			i.putExtra(EnumPreferences.NAME.toString(), mInputNome.getText()
					.toString().trim());
			i.putExtra(EnumPreferences.EMAIL.toString(), mInputEmail.getText()
					.toString().trim());
		}

		i.putExtra(EnumPreferences.USUARIO_REGISTRADO.toString(), registredUser);
		i.putExtra(EnumPreferences.LISTA_DE_MENSAGENS.toString(), Util
				.getPreference(getApplicationContext(),
						EnumPreferences.LISTA_DE_MENSAGENS.toString(),
						String.class));
		Log.d("RegisterActivity", "onClick para enviar.");
		startActivity(i);
		finish();
	}

	/**
	 * BroadcastReceiver, para receber as mensagens enviadas pelo servidor, e
	 * atualizar a UI com os valores recebidos.
	 */
